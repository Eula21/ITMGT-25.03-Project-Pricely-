...
from .utils import run_all_scrapers # EULZ JUL 16 | imported the multi-scraper thingy

from django.http import JsonResponse # EUL JUL 16
...
def search_results(request):
    ...
    # EULZ JUL 16 | Used Celery-based concurrent scrapers instead of single-source scraper
    products = run_all_scrapers(query) if query else []

    # Manual filtering (since it's a list, not a QuerySet)
...
def check_task_status(request):
    ...
    if result.status == 'SUCCESS':
        return JsonResponse({
            'status': 'done',
            'result': result.result  # already a Python list/dict
        })
