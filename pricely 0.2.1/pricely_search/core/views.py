...
def search_results(request):
    ...
    # Grab the data submitted by the users
    query = request.GET.get("query", "")
    max_price = request.GET.get("max_price")
    min_price = request.GET.get("min_price")
    min_rating = request.GET.get("min_rating")

    products = Product.objects.all() # Grab data from the Product model
    ...
    if query:
        ...
        unique_history = []
        for item in history:
            key = tuple(item.items()) # Convert the dictionary to a tuple for hashing
            if key not in seen:
                seen.add(key)
                unique_history.append(item)
        request.session["search_history"] = unique_history[:10]  # Save the updated history back to the session (limit to last 10)
